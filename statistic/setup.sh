...
function Installation() {
    ...
    cp statistic.py ${DESTDIR}
    checkCode 11 "Copy statistic.py failed." > /dev/null
    ...
    if [[ ! -f "${DESTDIR}/config.json" ]]; then
      cp conf-template.json ${DESTDIR}/config.json
      checkCode 11 "Copy conf-template.json failed." > /dev/null
    else
      cp conf-template.json ${DESTDIR}
      checkCode 11 "Copy conf-template.json failed." > /dev/null
    fi
